...
def create_account():
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            # Extract face from original full-resolution gray frame
            full_gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            face_img = full_gray[y:y+h, x:x+w]
...
def login():
    ...
    frame_count = 0
    last_faces = []  # Store last detected faces for smooth tracking
    ...
    while True:
        ...
        # Detect faces every 2nd frame for better performance
        frame_count += 1
        if frame_count % 2 == 0:
            # Resize for faster processing
            small_frame = cv2.resize(frame, (0, 0), fx=0.5, fy=0.5)
            gray = cv2.cvtColor(small_frame, cv2.COLOR_BGR2GRAY)
            detected_faces = face_cascade.detectMultiScale(gray, 1.2, 4)
            # Scale coordinates back and store
            last_faces = [(x*2, y*2, w*2, h*2) for (x, y, w, h) in detected_faces]
        
        # Always use the most recent face detections for drawing
        for (x, y, w, h) in last_faces:
            # Get face from full resolution for better recognition
            full_gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            face_img = full_gray[y:y+h, x:x+w]
            ...
            if confidence < 70:
                ...
                # Show unlock status
                cv2.putText(frame, "DOOR UNLOCKED - Press Q to exit", (10, 30),
        ...
        # Show instructions if no faces detected
        if len(last_faces) == 0:
            cv2.putText(frame, "Looking for face... (Press Q to exit)", (10, 30),
